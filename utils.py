...
#             plt.axis("on")
            ...
            if not os.path.exists('output/img_train_val/{}'.format(self.epoch)):
                os.makedirs('output/img_train_val/{}'.format(self.epoch))
            
            img_list = []
            
            for image_name, tensor in images.items():
                img_list.append(0.5*(tensor[0] + 1.0))
            
            save_image(torch.cat((img_list[0], img_list[1], img_list[2], img_list[3]), dim=2), 'output/img_train_val/{}/{}_{}.png'.format(self.epoch, self.mode, self.epoch))
